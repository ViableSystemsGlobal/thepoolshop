...
export default function AddDistributorModal({ isOpen, onClose, onSuccess }: AddDistributorModalProps) {
  ...
  const loadProducts = async () => {
    try {
      ...
      const response = await fetch('/api/products?limit=1000', { credentials: 'include' });
      ...
      if (response.ok) {
        ...
        if (!data.products || data.products.length === 0) {
          ...
          toastError('No products available. Please add products first.');
        } else {
          console.log('✅ Products loaded successfully:', data.products.length, 'products');
          success(`Loaded ${data.products.length} products successfully`);
        }
      } else {
      ...
      toastError('Error loading products');
    } finally {
  ...
  const handleInputChange = (field: string, value: string | string[] | File | File[]) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const handleFileUpload = (field: 'profileImage' | 'businessDocuments' | 'idDocuments', files: FileList | null) => {
    if (!files) return;
  ...
  const removeFile = (field: 'businessDocuments' | 'idDocuments', index: number) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log('🚀 Submitting distributor data:', submitData);
      console.log('📊 Interested products count:', formData.interestedProducts?.length || 0);
      ...
      if (response.ok) {
        ...
        success('Distributor added successfully!');
        onSuccess();
        onClose();
        resetForm();
      } else {
        ...
        toastError(errorData.error || 'Failed to add distributor');
      }
    } catch (error) {
      console.error('Error adding distributor:', error);
      toastError('An error occurred while adding the distributor');
    } finally {
      setLoading(false);
    }
  };
    ...
    <Modal isOpen={isOpen} onClose={onClose} size="lg">
      <div className="p-6">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold">Add New Distributor</h2>
          <Button
            onClick={onClose}
            variant="ghost"
            size="sm"
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-gray-600">
                        Available: {products.length} products | Loading: {productsLoading ? 'Yes' : 'No'}
                      </span>
                      <Button
                        type="button"
                        variant="outline"
                    ...
                    <MultiSelect
                      ...
                      placeholder={products.length === 0 ? "No products available" : "Select products of interest"}
                    />
                  </div>
              )}
            </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="mt-2">
                  <input
                    ...
                    onChange={(e) => handleFileUpload('businessDocuments', e.target.files)}
              ...
              <div>
                ...
                <div className="mt-2">
                  ...
                  />
                  {formData.idDocuments.length > 0 && (
                    <div className="mt-2 space-y-1">
                      {formData.idDocuments.map((file, index) => (
                        <div key={index} className="flex items-center justify-between bg-gray-50 p-2 rounded">
                          <span className="text-sm">{file.name}</span>
                          <Button
                            onClick={() => removeFile('idDocuments', index)}
                            variant="ghost"
                            size="sm"
                            className="p-1 h-auto"
                          >
