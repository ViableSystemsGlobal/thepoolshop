...
async function runCreditMonitoring() {
  try {
    console.log('[2025-09-24T10:20:00.000Z] 🔄 AUTOMATIC: Processing credit monitoring...');
    console.log('Starting credit monitoring processing...');
    ...
    if (monitoringEnabled !== 'true') {
      ...
      console.log('[2025-09-24T10:20:00.000Z] ✅ AUTOMATIC: Credit monitoring skipped (disabled)');
    ...
    console.log(`📊 Found ${distributors.length} distributors to check for credit alerts`);
...
AdPools Credit Monitoring System
    ...
    console.log(`📊 Found ${alertsSent} credit alerts and sent notifications`);
    console.log('Credit monitoring processing completed');
    console.log('[2025-09-24T10:20:00.000Z] ✅ AUTOMATIC: Credit monitoring processing completed');
    ...
    console.log('[2025-09-24T10:20:00.000Z] ❌ AUTOMATIC: Credit monitoring processing failed');
    throw error;
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('🚀 Manual credit monitoring task triggered by:', session.user.email);

    const result = await runCreditMonitoring();
    ...
    console.error('Error in credit monitoring task:', error);
    return NextResponse.json(
      { 
        error: 'Failed to run credit monitoring task',
